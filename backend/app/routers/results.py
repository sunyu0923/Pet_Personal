...
def submit_results(body: SubmitAnswersRequest, db: Session = Depends(get_db)):
    ...
    if len(questions) != 12:
        raise HTTPException(status_code=400, detail="Must submit exactly 12 answers for the correct pet type")

    scored = score_answers(questions, body.answers)

    personality = db.query(PersonalityType).filter(
        PersonalityType.pet_type == body.pet_type,
        PersonalityType.mbti_code == scored["mbti_code"],
    ).first()
...
def get_result(share_id: str, db: Session = Depends(get_db)):
    ...
    personality = db.query(PersonalityType).filter(
        ...
        PersonalityType.mbti_code == result.mbti_code,
    ).first()
